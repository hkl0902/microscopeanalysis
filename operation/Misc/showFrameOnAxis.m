...
try
  ...
  arrsize = size(hChild);
  num_elements = arrsize(1);
  disp('Num Elements');
  disp(num_elements);
  if(num_elements > 1)
    disp('Type of Each object in hChild');
    for i = 1:(num_elements + 1)
      disp(get(hChild(i), 'type'));
      if(strcmp(get(hChild(i), 'type'), 'image'))
          disp('Converting hChild to image');
          hChild = hChild(i);
          break;
      end
    end
...
% Check frame size
disp('Previous frame size');
disp(size(get(hIm, 'cdata')));
prev_size = size(get(hIm, 'cdata'));
disp('Current frame size');
disp(frame_size);
if ~isequal(prev_size(1:2), frame_size(1:2))
